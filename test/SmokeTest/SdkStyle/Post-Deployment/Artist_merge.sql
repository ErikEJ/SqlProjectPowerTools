-- EXEC [#sp_generate_merge] @schema = 'dbo', @table_name = 'Artist', @results_to_text = 1, @include_use_db = 0
-- Add the following ItemGroup to your project file to include the merge script in your project:
-- <ItemGroup>
--   <PostDeploy Include="Post-Deployment\Artist_merge.sql" />
-- </ItemGroup>
...
MERGE INTO [dbo].[Artist] WITH (SERIALIZABLE) AS [Target]
USING (VALUES
  (1,N'AC/DC')
 ,(2,N'Accept')
 ,(3,N'Aerosmith')
...
IF @mergeError <> 0 PRINT 'ERROR OCCURRED IN MERGE FOR [dbo].[Artist]' + CONCAT(' (SQL Server error code: ', @mergeError) + ')';
PRINT CONCAT('[dbo].[Artist] rows affected by MERGE: ', @mergeCount) + CONCAT(' (Inserted: ', @mergeCountIns) + CONCAT('; Updated: ', @mergeCountUpd) + CONCAT('; Deleted: ', @mergeCountDel) + ')';
GO
